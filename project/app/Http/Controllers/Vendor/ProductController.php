...
class ProductController extends Controller {
    ...
    public function uploadUpdate(Request $request, $id) {
        ...
        Storage::disk('dos')->put($path, $image);
        if ($data->photo != null) {
            if (file_exists(Storage::disk('dos')->url('assets/images/products/' . $data->photo))) {
                unlink(Storage::disk('dos')->url('assets/images/products/' . $data->photo));
            }
        }
        $input['photo'] = $image_name;
        $data->update($input);
        if ($data->thumbnail != null) {
            if (file_exists(Storage::disk('dos')->url('assets/images/thumbnails/' . $data->thumbnail))) {
                unlink(Storage::disk('dos')->url('assets/images/thumbnails/' . $data->thumbnail));
            }
        }

        $url = Storage::disk('dos')->url('assets/images/products/' . $data->photo);
        $getimage = file_get_contents($url);
        $img = Image::make($getimage)->resize(285, 285)->encode('jpg');
        $thumbnail = time() . str_random(8) . '.jpg';
        $path = 'assets/images/thumbnails/' . $thumbnail;
...
//                $input['childcategory_id'] = $request->childcategory_id;
                            ...
                            $img = Image::make($line[5])->resize(285, 285)->encode('jpg');
                            $thumbnail = time() . str_random(8) . '.jpg';
                            $path = 'assets/images/thumbnails/' . $thumbnail;
                            Storage::disk('dos')->put($path, $img);
    ...
    public function store(Request $request) {
        ...
        if ($prods < $package->allowed_products || $package->allowed_products == 0) {
            ...
            if ($file = $request->file('file')) {
                ...
                Storage::disk('dos')->putFileAs('/assets/files/', $file, $name);
            ...
            if ($file = $request->file('photo')) {
                $name = time() . $file->getClientOriginalName();
                Storage::disk('dos')->putFileAs('/assets/images/products/', $file, $name);
            ...
            $url = Storage::disk('dos')->url('assets/images/products/' . $prod->photo);
            $getimage = file_get_contents($url);
            $img = Image::make($getimage)->resize(800, 800)->encode('jpg');
...
//
            ...
            if ($files = $request->file('gallery')) {
                foreach ($files as $key => $file) {
                    if (in_array($key, $request->galval)) {
                        ...
                        $path = 'assets/images/galleries/' . $name;
                        Storage::disk('dos')->put($path, $img);
                        $gallery['photo'] = $name;
    ...
    public function update(Request $request, $id) {
        ...
        if ($request->type_check == 1) {
            ...
            if ($data->file != null) {
                if (file_exists(Storage::disk('dos')->url('assets/files/' . $data->file))) {
                    unlink(Storage::disk('dos')->url('assets/files/' . $data->file));
                }
            }
            $input['file'] = null;
        }
    ...
    public function destroy($id) {
        ...
        if (file_exists(Storage::disk('dos')->url('/assets/images/thumbnails/' . $data->thumbnail)) && $data->thumbnail != "") {
            unlink(Storage::disk('dos')->url('/assets/images/thumbnails/' . $data->thumbnail));
        }
        if ($data->file != null) {
            if (file_exists(Storage::disk('dos')->url('/assets/files/' . $data->file))) {
